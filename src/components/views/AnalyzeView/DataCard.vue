...
const processedData = computed(() => {
  ...
  return props.data.map((d) => {
    ...
    return {
      ...d,
      regression: inSelection ? (regression.slope * ts + regression.intercept) : Number.NaN,
    }
  })
})
...
const regressionResult = computed<RegressionResult | null>(() => {
  ...
  const selected = props.data
    .filter((d) => {
      ...
      return Number.isFinite(ts) && Number.isFinite(y) && ts >= min && ts <= max
    })
    .map((d) => ({
...
const formattedSlope = computed(() => regressionResult.value?.slope.toFixed(6) ?? '—')
const formattedIntercept = computed(() => regressionResult.value?.intercept.toFixed(2) ?? '—')
const formattedRSquared = computed(() => regressionResult.value?.rSquared.toFixed(4) ?? '—')
...
function getNearestTimestamp(target: number): number {
  ...
  const timestamps = props.data.map((d) => Number(d.timestamp)).filter((t) => Number.isFinite(t))
...
function handleBrushMove(selection: [number, number], _event: any, userDriven: boolean) {
...
onMounted(() => {
  if (chartContainer.value)
    autoAnimate(chartContainer.value)
})
</script>

<template>
  <div class="rounded-lg border bg-card p-3 sm:p-4 shadow-sm space-y-3 sm:space-y-3 " ref="chartContainer">
    <div class="flex items-center justify-between">
      <div class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{{ title }}</div>
    ...
    <div class="chart-container mb-8">
      <ChartContainer :config="chartConfig" class="aspect-auto w-full" :style="{ height: chartHeight }" :cursor="false">
        <VisXYContainer :data="processedData" :margin="{ left: -32 }" :y-domain="[0, yMax]">
          <VisLine :x="(d: any) => d.timestamp" :y="(d: any) => d[dataKey]" :color="chartConfig[dataKey]?.color"
            :line-width="2" />
          <VisLine v-if="activeTool === 'linear_regression'" :x="(d: any) => d.timestamp" :y="(d: any) => d.regression"
            color="#f97316" :line-width="2" />

          <VisBrush v-if="activeTool === 'linear_regression' || activeTool === 'delete'" :selectionMinLength="2"
            :selection="brushSelection" :draggable="true" :onBrushEnd="handleBrushMove" />

          <VisAxis type="x" :x="(d: any) => d.timestamp" :tick-line="false" :domain-line="false" :grid-line="false"
    ...
    <div class="mt-2 grid grid-cols-3 gap-2" v-if="activeTool === 'linear_regression'">
      <div class="rounded-md border text-xs px-3 py-2 flex flex-col items-center bg-muted">
